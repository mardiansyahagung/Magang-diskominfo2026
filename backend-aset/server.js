...
db.connect((err) => {
  if (err) {
    ...
    db.query(createLogTableQuery, (logTableErr) => {
      if (logTableErr) {
        console.error("Gagal memastikan tabel log_aktivitas:", logTableErr.message);
        return;
      }
    });
  }
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    db.query('SELECT COUNT(*) AS total FROM users', async (countErr, countResults) => {
      if (countErr) {
        console.error("Error cek jumlah user:", countErr.message);
        return res.status(500).json({ message: 'Error server saat memeriksa data user' });
      }
        ...
        (err) => {
          if (err) {
            ...
            if (err.code === 'ER_DUP_ENTRY') {
              return res.status(400).json({ message: 'Username sudah digunakan, silakan pakai username lain.' });
            }
            return res.status(400).json({ message: `Gagal registrasi: ${err.message}` });
          }
          return res.json({
            success: true,
...
app.get('/api/me', (req, res) => {
  ...
  return res.json({ username: req.session.username, role: req.session.role || 'user' });
});
...
app.put('/api/users/:id/role', requireAdmin, (req, res) => {
  ...
  db.query('SELECT username, role FROM users WHERE id = ?', [id], (findErr, findResults) => {
    if (findErr) return res.status(500).json({ message: 'Gagal memeriksa data user', error: findErr });
    if (findResults.length === 0) return res.status(404).json({ message: 'User tidak ditemukan' });

    const targetUsername = findResults[0].username;
    const targetSaatIni = findResults[0].role;

    if (targetSaatIni === 'admin' && role === 'user') {
      db.query("SELECT COUNT(*) AS total FROM users WHERE role = 'admin'", (cntErr, cntResults) => {
        if (cntErr) return res.status(500).json({ message: 'Gagal memeriksa jumlah admin', error: cntErr });
        if (cntResults[0].total <= 1) {
          return res.status(400).json({ message: 'Tidak bisa menurunkan admin terakhir. Tunjuk admin lain dahulu.' });
        }
        ubahRole();
      });
    } else {
    ...
    function ubahRole() {
      db.query('UPDATE users SET role = ? WHERE id = ?', [role, id], (err) => {
        if (err) return res.status(500).json({ message: 'Gagal mengubah role', error: err });
        catatLog(req, 'ubah_role', 'users', id, `Mengubah role user "${targetUsername}" menjadi ${role}`);
...
app.delete('/api/users/:id', requireAdmin, (req, res) => {
  ...
  db.query('SELECT username, role FROM users WHERE id = ?', [id], (findErr, findResults) => {
    ...
    if (findResults[0].role === 'admin') {
      return res.status(400).json({ message: 'Akun admin tidak bisa dihapus. Turunkan role-nya dahulu jika perlu.' });
    }
    ...
    db.query('DELETE FROM users WHERE id = ?', [id], (err) => {
      ...
      catatLog(req, 'hapus_user', 'users', id, `Menghapus user "${targetUsername}"`);
...
app.get("/api/aset", requireLogin, (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) return res.status(500).json({ message: "Gagal mengambil data aset", error: err });
    res.json(results);
  });
});

app.post("/api/aset", requireAdmin, (req, res) => {
  const { nama_domain, jenis_aset, is_aktif, jenis_anomali, tanggal_insiden, catatan, status_perbaikan } = req.body;
  ...
  db.query(
    ...
    (err, results) => {
      if (err) return res.status(500).json({ message: "Gagal menambah aset", error: err });
      catatLog(req, "tambah", "aset_web", results.insertId, `Menambahkan aset baru: ${nama_domain}`);
...
app.put("/api/aset/pulihkan/:id", requireAdmin, (req, res) => {
  ...
  db.query("SELECT nama_domain FROM aset_web WHERE id = ?", [id], (findErr, findResults) => {
    if (findErr) return res.status(500).json({ message: "Gagal memeriksa data aset", error: findErr });
    ...
    db.query(query, [id], (err, results) => {
      if (err) return res.status(500).json({ message: "Gagal memulihkan aset", error: err });
      catatLog(req, "pulihkan", "aset_web", id, `Memulihkan aset "${namaDomain}" menjadi Aman`);
...
app.put("/api/aset/:id", requireAdmin, (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) return res.status(500).json({ message: "Gagal mengupdate aset", error: err });
      catatLog(req, "edit", "aset_web", id, `Mengedit data aset: ${nama_domain}`);
...
app.delete("/api/aset/:id", requireAdmin, (req, res) => {
  ...
  db.query("SELECT nama_domain FROM aset_web WHERE id = ?", [id], (findErr, findResults) => {
    ...
    db.query(query, [id], (err, results) => {
      if (err) return res.status(500).json({ message: "Gagal menghapus aset", error: err });
      catatLog(req, "hapus", "aset_web", id, `Menghapus aset "${namaDomain}"`);
